...
function TimeSelect({ label, value, onChange }: { label: string; value: string; onChange: (value: string) => void }) {
  return <label className={s.timeField}><span>{label}</span><select value={value} onChange={e => onChange(e.target.value)} required><option value="">Choose time</option>{TIMES.map(time => <option key={time} value={time}>{timeLabel(time)}</option>)}</select></label>;
}
export function StallDetails({ name, setName, hours, setHours, sameHours, setSameHours, photo, onPhoto, photoUrl, busy, onExtract, onManual, onExample, onExisting, currentMenu, existingAvailable, pageDetails, setPageDetails }: {
  ...
  busy: boolean; onExtract: () => void; onManual: () => void; onExample: () => void; onExisting: () => void; currentMenu: string; existingAvailable: boolean;
}) {
  ...
  return <div className={s.stack}>
    <section className={`card ${s.section}`} aria-labelledby="stall-title">
      ...
      <div className={s.hours}>
        ...
        {(sameHours ? [hours[0]] : hours).map((day, index) => <div className={s.day} key={index}>
          {(!sameHours || day.closed) && <div className={s.dayHeading}><strong>{sameHours ? "Every day" : DAYS[index]}</strong><label className={s.check}><input type="checkbox" checked={day.closed} onChange={e => patchDay(index, { closed: e.target.checked })} />Closed</label></div>}
          {!day.closed && <>
        ...
        <p className={s.help}>Hours are kept in this preview for now. Saving hours to your live stall page is still being connected.</p>
      </div>
    </section>
    ...
    <section className={`card ${s.section}`} aria-labelledby="stall-page-title">
      ...
      <p className={s.help}>Address and contact are for this page only until stall details can be saved.</p>
    </section>
  </div>;
}
