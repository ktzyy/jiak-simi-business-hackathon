...
export function MenuReview({ polishing = false, polishAction, manageAvailability = false, sharedRows, setSharedRows, excludedExtras, setExcludedExtras, anyExtras, onAnyExtrasChange, dishes, draft, uploads, onDishChange, onConfirmDish, onAddDish, onRemoveDish, onContinue, onBack, renderDishPhoto }: {
  ...
  renderDishPhoto: (dish: DishEdit, addonsAction: ReactNode) => ReactNode;
}) {
  ...
  return <div className={s.stack} ref={root}>
    ...
    <div className={s.dishes}>{dishes.map((dish, index) => {
      ...
      return <article key={dish.id} className={`${s.dish} ${!dish.included ? s.excluded : ""}`}>
        ...
        <div className={s.dishSummary}>
          <span className={s.dishNumber}>{String(index + 1).padStart(2, "0")}</span>
          <label className={s.summaryMain}><span className={s.srOnly}>Dish name</span><input className={s.inlineName} aria-invalid={attempted && badName(dish.name)} value={dish.name} maxLength={120} placeholder="Dish name" onChange={e => onDishChange(dish.id, { name: e.target.value })} /></label>
          <label className={s.inlinePrice}><span aria-hidden="true">S$</span><span className={s.srOnly}>Price (S$)</span><input aria-invalid={attempted && badPrice(dish.price)} value={dish.price} inputMode="decimal" placeholder="0.00" onChange={e => onDishChange(dish.id, { price: e.target.value })} /></label>
        </div>
        {dish.included && renderDishPhoto(dish, <button type="button" className="btn btn-outline" aria-expanded={isOpen} aria-controls={`dish-${dish.id}`} onClick={() => setExpanded(isOpen ? null : dish.id)}>{isOpen ? "Close add-ons" : "+ Add-ons"}</button>)}
        {manageAvailability && <label className={`${s.check} ${s.availability}`}><input type="checkbox" checked={!dish.available} onChange={e => onDishChange(dish.id, { available: !e.target.checked })} />Sold out</label>}
    ...
    <section className={`card ${s.section}`}>
      ...
      {globalRows.map((row, index) => <div key={row.id} className={s.sharedOptionRow}>
        <label className={s.check}><input type="checkbox" checked={row.included} onChange={e => updateGlobal(index, { included: e.target.checked })} /><span className={s.srOnly}>Include {row.name || "add-on"}</span></label>
