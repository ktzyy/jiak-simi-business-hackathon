...
export function useDishPhotos(restaurantId: string) {
  ...
  async function accept(id: string, record: PhotoRecord, job: DishPhotoJob, token: string, epoch: number) {
    ...
    const next = { ...record, jobId: job.jobId, status: job.status, candidate: job.candidate ?? undefined };
    update(id, next);
  ...
  return { records, previews, busy, errors, storageError, generate, check, select, remove, reset };
}
