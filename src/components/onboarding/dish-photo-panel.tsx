...
export function DishPhotoPanel({ savedPhoto, dishId, dishName, crop, upload, sourceEntryId, currentCandidate, selected, busy, error, onCrop, onGenerate, onSelect, onRemove, onCheck }: {
  ...
  return <section className={s.panel} aria-label={`Photo for ${dishName || "new dish"}`} aria-busy={busy}>
    <div className={s.comparison}>
      ...
      {photo && <div className={s.photoColumn}><figure><img src={photo.previewUrl} alt={`${photo.candidate.mode === "generate_similar" ? "Generated" : "Polished"} ${dishName}`} /><figcaption>{`${photo.candidate.mode === "generate_similar" ? "AI-generated" : "AI-enhanced"}${selected ? " · selected" : ""}`}</figcaption></figure><p className={s.cropHint}>Check ingredients and portion.</p><button type="button" className="btn btn-teal" disabled={busy} onClick={selected ? onRemove : onSelect}>{selected ? crop ? "Use original" : "Remove photo" : photo.candidate.mode === "generate_similar" ? "Use photo" : "Use polished"}</button></div>}
    </div>
