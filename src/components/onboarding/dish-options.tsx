...
import { amount, type GroupEdit } from "./review-state";
...
export function DishOptions({ showErrors = false, groups, onChange, anyExtras, onAnyExtrasChange }: { showErrors?: boolean; groups: GroupEdit[]; onChange: (groups: GroupEdit[]) => void; anyExtras: ReadonlySet<string>; onAnyExtrasChange: (id: string, enabled: boolean) => void }) {
  ...
  return <section aria-label="Add-ons">
    ...
    {groups.map(group => {
      ...
      return <fieldset className={s.addonCard} key={group.id} aria-label={group.name || "Add-ons group"}>
        ...
        {!PRESETS.includes(group.name) && <label className="field"><span className={s.srOnly}>Heading</span><input aria-label="Heading" aria-invalid={showErrors && !group.name.trim()} value={group.name} maxLength={120} placeholder="Heading" onChange={e => patch(group.id, { name: e.target.value })} /></label>}
        ...
        {group.options.map((option, index) => <div key={option.id} className={s.addonRow}>
          <label className="field"><span className={s.srOnly}>Option {index + 1}</span><input aria-invalid={showErrors && !option.name.trim()} value={option.name} maxLength={120} placeholder="Add an option" onChange={e => patch(group.id, { options: group.options.map(o => o.id === option.id ? { ...o, name: e.target.value } : o) })} /></label>
          <label className="field"><span className={s.srOnly}>Extra price (S$)</span><input aria-invalid={showErrors && (!Number.isInteger(amount(option.price, true)) || Math.abs(amount(option.price, true)) > 1_000_000)} value={option.price} inputMode="decimal" placeholder="0.00" onChange={e => patch(group.id, { options: group.options.map(o => o.id === option.id ? { ...o, price: e.target.value } : o) })} /></label>
        ...
        <button type="button" className={s.textAction} data-invalid={showErrors && !group.options.length} disabled={group.options.length >= 20} onClick={() => patch(group.id, { options: [...group.options, { id: crypto.randomUUID(), name: "", price: "0.00" }], ...(rule === "any" ? { max: String(group.options.length + 1) } : {}) })}>+ Add option</button>
        <div className={s.addonFooter}><label className="field">Customer can choose<select aria-invalid={showErrors && (!/^\d+$/.test(group.min) || !/^\d+$/.test(group.max) || Number(group.min) > Number(group.max) || Number(group.max) > group.options.length)} value={rule} onChange={e => { onAnyExtrasChange(group.id, e.target.value === "any"); if (e.target.value === "any") patch(group.id, { min: "0", max: String(group.options.length) }); if (e.target.value === "optional-one") patch(group.id, { min: "0", max: "1" }); if (e.target.value === "required-one") patch(group.id, { min: "1", max: "1" }); }}><option value="any">Multiple · optional</option><option value="optional-one">One · optional</option><option value="required-one">One · required</option>{rule === "custom" && <option value="custom">Custom rule</option>}</select></label>
