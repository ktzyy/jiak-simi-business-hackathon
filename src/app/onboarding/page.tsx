...
export default async function OnboardingPage({ searchParams }: { searchParams: Promise<{ restaurantId?: string | string[] }> }) {
  const query = (await searchParams).restaurantId;
  ...
  return <Onboarding key={parsed.data} restaurantId={parsed.data} />;
}
