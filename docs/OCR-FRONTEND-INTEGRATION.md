...
4. In the stall-name section, explicitly review and save all seven days of opening hours using `saveStallDetails(restaurantId, {expectedVersion,name,timezone:"Asia/Singapore",weeklyHours}, staffAccessToken)`. Initial `expectedVersion` is 0; later saves use the current version. See [UI contracts](KIMBERLEY-UI-CONTRACTS.md) for the full hours shape.
5. Preview the reviewed menu and saved stall details together, then call `publishMenu(menu, staffAccessToken, stallDetailsVersion)` only after the merchant presses Publish. The endpoint validates membership, menu structure and the current reviewed details version, and pins that immutable details snapshot to the publication. A new extraction needs new review decisions. The current demo publication is menu v2 with details v1, explicitly approved for 09:00–18:00 Monday–Sunday in Asia/Singapore. These are demo-specific reviewed values, not inferred OCR output or defaults. Legacy publications may still return `details:null`.
...
These review notes are local workflow evidence. The current publish API accepts `{menu,stallDetailsVersion}`; it does not persist OCR source/review notes or claim durable photo provenance. Do not tell merchants that uploaded sources or review history are saved across sessions. Optional protected image storage and photo enhancement need the separately scoped contract/workflow.
...
The web extension `20260913053454_jiak_simi_web_ordering.sql` is applied. Kimberley owns the onboarding/stall-name hours controls, review and publication preview UI. New ordering carts require explicit `fulfillmentType`; kitchen Done/counts are persisted API features, described in [UI contracts](KIMBERLEY-UI-CONTRACTS.md). Photo extraction does not choose fulfillment or opening hours.

The approved v2 demo menu adds optional Egg +100 cents, Char Siew +200 cents and Shao Rou +200 cents to every dish, plus an optional mutually exclusive free Chilli/No chilli group. Use `DEMO_MENU_WITH_EXTRAS` for that exact fixture; `DEMO_MENU` preserves historical v1. Read current menu/details versions with `readPublishedStall` before review/publication. The joint base remains 1400 cents; one Char Siew Rice with all three extras is 950 cents. These approved demo additions are separate from uncertain OCR evidence.
